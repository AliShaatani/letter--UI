...
import { ShieldCheck, Archive, RotateCcw, Building2 } from 'lucide-react';

export const EmptyState = () => {
  const { setActiveView, resetToMockData } = useCorrespondenceStore();
      ...
      <div className="max-w-lg w-full bg-white p-8 sm:p-10 rounded-3xl border border-[#E2E6EC] shadow-lg animate-fade-in relative overflow-hidden">
        ...
        <div className="space-y-3 max-w-sm mx-auto">
          <button
            ...
            className="w-full flex items-center justify-center gap-2 py-3 px-5 text-xs font-bold font-cairo rounded-xl bg-gradient-to-r from-[#1B4B8A] to-[#123a6b] text-white shadow-sm hover:shadow-md transition-all border border-[#C8952A]/40"
          >
            <Archive className="w-4 h-4 text-[#E0B863]" />
            <span>استعراض الأرشيف الإلكتروني (المعاملات المكتملة)</span>
          </button>

          <button
            onClick={resetToMockData}
            className="w-full flex items-center justify-center gap-2 py-2.5 px-5 text-xs font-semibold text-gray-600 bg-gray-50 hover:bg-gray-100 rounded-xl border border-gray-200 transition-colors"
          >
            <RotateCcw className="w-3.5 h-3.5 text-gray-500" />
            <span>إعادة تحميل المعاملات الافتراضية للاختبار</span>
          </button>
        </div>
      </div>
    </div>
